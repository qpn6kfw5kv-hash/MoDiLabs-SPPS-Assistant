...
const CYCLIZATION_PROTECTED_AMINO_ACIDS = {
  K: {
    code3: 'Lys',
    fmocName: 'Fmoc-Lys(Alloc)-OH',
    fmocMW: 452.50,
    protection: 'Alloc'
  },
  D: {
    code3: 'Asp',
    fmocName: 'Fmoc-L-Asp(OAll)-OH',
    aliases: ['Fmoc-Asp(OAll)-OH'],
    fmocMW: 395.41,
    protection: 'OAll'
  }
};
...
function stripCyclizationMarkers(seq) {
  return String(seq || '').replace(/\*/g, '');
}

function getResidueDisplayName(token) {
  if (AMINO_ACIDS[token]) return AMINO_ACIDS[token].code3;
  return String(token || '').trim();
}

function getCyclizationEndpointType(token) {
  const raw = String(token || '').trim();
  const upper = raw.toUpperCase();
  const lettersOnly = upper.replace(/[^A-Z]/g, '');

  if (upper === 'C' || lettersOnly.indexOf('CYS') !== -1) return 'thiol';
  if (upper === 'K' || /(LYS|ORN|DAP|DAB)/.test(lettersOnly)) return 'amine';
  if (upper === 'D' || upper === 'E' || /(ASP|GLU)/.test(lettersOnly)) return 'carboxyl';
  return 'unknown';
}

function inferCyclization(startToken, endToken) {
  const startType = getCyclizationEndpointType(startToken);
  const endType = getCyclizationEndpointType(endToken);

  if (startType === 'thiol' && endType === 'thiol') {
    return {
      type: 'disulfide',
      label: 'Ponte disolfuro',
      massDelta: -2.016,
      note: 'Perdita di 2 H'
    };
  }
  ...
  if (
    (startType === 'amine' && endType === 'carboxyl') ||
    (startType === 'carboxyl' && endType === 'amine')
  ) {
    return {
      type: 'amide',
      label: 'Ponte ammidico side-chain',
      massDelta: -18.015,
      note: 'Perdita di H2O'
    };
  }
  ...
  return {
    type: 'unknown',
    label: 'Ciclizzazione marcata',
    massDelta: 0,
    note: 'Tipo non riconosciuto: delta massa non applicato'
  };
}

function analyzeCyclizations(seq) {
  ...
  for (let i = 0; i < raw.length; i++) {
    if (raw[i] === '*') markerPositions.push(i);
  }
  ...
  if (markerPositions.length % 2 !== 0) {
    warnings.push('Gli asterischi di ciclizzazione devono essere in coppia: usa *inizio* e *fine*.');
  }

  for (let i = 0; i + 1 < markerPositions.length; i += 2) {
    const segment = raw.slice(markerPositions[i] + 1, markerPositions[i + 1]).trim();
    const tokens = tokenizeSequence(segment);

    if (tokens.length < 2) {
      warnings.push('La porzione ciclizzata deve contenere almeno due residui.');
      cyclizations.push({
        segment,
        tokens,
        startToken: '',
        endToken: '',
        label: 'Ciclizzazione non valida',
        massDelta: 0,
        note: 'Porzione troppo corta'
      });
      continue;
    }
    ...
    const inferred = inferCyclization(startToken, endToken);

    if (inferred.type === 'unknown') {
      warnings.push('Ciclizzazione marcata tra ' + getResidueDisplayName(startToken) + ' e ' + getResidueDisplayName(endToken) + ': tipo non riconosciuto, delta massa non applicato.');
    }

    cyclizations.push({
      segment,
      tokens,
      segmentStartTokenIndex: tokenizeSequence(raw.slice(0, markerPositions[i])).length,
      segmentEndTokenIndex: tokenizeSequence(raw.slice(0, markerPositions[i])).length + tokens.length - 1,
      startToken,
      endToken,
      startLabel: getResidueDisplayName(startToken),
      endLabel: getResidueDisplayName(endToken),
      type: inferred.type,
      label: inferred.label,
      massDelta: inferred.massDelta,
      note: inferred.note
    });
  }
  ...
  return {
    ...
    isValid: warnings.every(w => w.indexOf('devono essere in coppia') === -1 && w.indexOf('almeno due residui') === -1),
    cyclizations,
    warnings,
    totalMassDelta: cyclizations.reduce((sum, c) => sum + (parseFloat(c.massDelta) || 0), 0),
    cleanSequence: stripCyclizationMarkers(raw)
  };
}

function getCyclizationProtectedOverrides(seq) {
  const info = analyzeCyclizations(seq);
  const overrides = {};

  for (const cycle of info.cyclizations) {
    if (cycle.type !== 'amide') continue;

    const startVariant = CYCLIZATION_PROTECTED_AMINO_ACIDS[cycle.startToken];
    const endVariant = CYCLIZATION_PROTECTED_AMINO_ACIDS[cycle.endToken];

    if (startVariant) {
      overrides[cycle.segmentStartTokenIndex] = Object.assign({ token: cycle.startToken }, startVariant);
    }
    if (endVariant) {
      overrides[cycle.segmentEndTokenIndex] = Object.assign({ token: cycle.endToken }, endVariant);
    }
  }
 ...
 */
function calculatePeptideMW(tokens, cTerminus = 'amide', customMWs = {}, sequence = '') {
  let mw = 0;
  ...
  if (sequence) {
    const cyclizationInfo = analyzeCyclizations(sequence);
    if (cyclizationInfo.isValid) mw += cyclizationInfo.totalMassDelta;
  }

...
function renderInventoryCheckRows(aaTokens, activatorNames, synthesis) {
  ...
  return uniqueArr.map(function(item) {
    ...
    if (inv) {
      // Look for the last pair of parentheses (often used for color/location notes)
      var colorMatch = inv.name.match(/.*\(([^)]+)\)[^()]*$/);
      var displayLocation = inv.location;
      if (colorMatch) displayLocation += ' (' + colorMatch[1] + ')';

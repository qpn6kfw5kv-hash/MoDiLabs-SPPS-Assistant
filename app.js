...
function renderDetail(s) {
  ...
  return `
    ...
    ${renderCyclizationSection(cyclizationInfo)}
...
function renderCyclizationSection(info) {
  ...
  var warnings = info.warnings.length > 0
    ...
    : '';
  var protocol = renderCyclizationProtocol(info);
...
function renderCyclizationProtocol(info) {
    ...
    return [
      {
        ...
        body: 'Deprotezione ortogonale su resina con catalisi Pd(0) e scavenger dell allyl group, in condizioni anidre/inerti secondo SOP del laboratorio.'
      },
      {
        title: 'Lavaggi e scavenging Pd',
        body: 'Dopo la deprotezione, lavare accuratamente la resina e includere uno step di scavenging del palladio prima della ciclizzazione.'
      },
      {
        title: 'Ciclizzazione lattamica',
        body: 'Attivare il carbossile laterale deprotetto e ciclizzare con sistema HATU/DIPEA, HBTU/DIPEA, PyBOP/DIPEA o PyAOP/DIPEA in DMF; ripetere se il test di completamento non e soddisfacente.'
      },
      {
        title: 'Controllo',
        body: 'Verificare consumo dell ammina libera con test colorimetrico quando applicabile e confermare la massa tramite mini-cleavage LC-MS.'
      }
    ];
  }

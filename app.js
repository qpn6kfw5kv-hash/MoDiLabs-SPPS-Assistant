...
function getInventorySearchAliases(value) {
  ...
  if (/fmocl?lysallocoh/.test(normalized) || /lysalloc/.test(normalized)) {
    aliases.push('fmoclysallocoh', 'fmocllysallocoh', 'lysalloc');
  }
...
function getCyclizationBuildingBlocks(cycle) {
  ...
  if (startVariant) blocks.push(startVariant.fmocName);
  if (endVariant) blocks.push(endVariant.fmocName);
  return blocks;
}

    ...
    return [
      {
        ...
        body: '0,015 eq Pd(PPh3)4, 3 eq DMBA (oppure morfolina se sono presenti Cys), DCM:DMF anidri 3:2. Primo ciclo: 15 min in condizioni anidre.'
      },
      {
...
function renderCyclizationPrintSection(info) {
  if (!info || !info.cyclizations || info.cyclizations.length === 0) return '';
...
function printSynthesis(id) {
  ...
  const cyclizationPrintHtml = cyclizationInfo.cyclizations.length > 0
    ...
    : '';
  const cyclizationPrintSection = renderCyclizationPrintSection(cyclizationInfo);

...
function printSynthesis(id) {
  ...
  tokens.forEach((token, i) => {
    ...
    const groupKey = [fmocName, mw.toFixed(2), isPreloadedStep ? 'preloaded' : 'coupling'].join('|');
  ...
  const rowsHtml = aminoAcidPrintGroups.map((group, i) => {
    ...
    return `<tr><td>${i + 1}. ${escapeHtml(group.name)}${note}${positionLabel}</td><td>${group.mw.toFixed(2)}</td><td>${quantity}</td></tr>`;
  }).join('');
